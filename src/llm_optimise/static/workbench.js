...
(() => {
  ...
  function renderForm() {
    ...
    if (state.tool === "feedback") {
      $("wb-f-runtime").addEventListener("change", feedbackVisibility);
      $("wb-f-acceptance_mode").addEventListener("change", feedbackVisibility);
    }
    contractVisibility(); feedbackVisibility(); refreshDynamicFields();
  }
  function feedbackVisibility() {
    if (state.tool !== "feedback") return;
    const runtime = $("wb-f-runtime").value;
    const component = runtime === "rust" || runtime === "python" && $("wb-f-acceptance_mode").value === "component";
    for (const [key, visible] of [["acceptance_mode", runtime === "python"], ["entrypoint", component], ["cases", component]]) {
      const input = $("wb-f-" + key); input.closest("label").hidden = !visible; input.disabled = !visible;
    }
    const entry = $("wb-f-entrypoint");
    if (["main.py", "main.rs"].includes(entry.value)) entry.value = runtime === "rust" ? "main.rs" : "main.py";
  }
  ...
  function jsonDetails(title, value) {
    ...
    details.addEventListener("toggle", () => {
      if (details.open && details.children.length === 1) details.append(el("pre", pretty(value)));
      else if (!details.open) details.querySelector("pre")?.remove();
    });
    return details;
  }
  function showResult(result, title, request = null) {
    ...
    if (result.worker?.predictions?.length) target.append(jsonDetails("Generated answers", result.worker.predictions));
    ...
    target.append(jsonDetails("Inspect complete evidence JSON", result));
    if (request) target.append(jsonDetails("Request used for this run", request));
  }
