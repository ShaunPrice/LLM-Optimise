...
The three generated recipes were checked against field names in Soup's configuration source. This was an AST/schema-field check, **not an executed training run or full semantic validation**. `data.max_length` belongs under `data`; streaming options belong under `training`. [Pinned schema](https://github.com/MakazhanAlpamys/Soup/blob/254351e10331e360b5d75e26aabe0a4e591e16a4/src/soup_cli/config/schema.py), [check artifact](../validation/soup-schema-check.json)
...
| [QLoRA](https://arxiv.org/abs/2305.14314) | Adapter training with a quantised frozen base | **Recipe integration.** Compare resident QLoRA with Soup streaming before increasing model size. No live fine-tuning result yet. |
